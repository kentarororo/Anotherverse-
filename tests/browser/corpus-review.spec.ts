...
test('renders and persists the fixed 60-chapter human review gate', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '60-chapter read-aloud review' })).toBeVisible();
  await expect(page.locator('.review-entry')).toHaveCount(60);
  await expect(page.locator('.review-metrics dd').first()).toHaveText('0/60');
  ...
  await expect(page.locator('.review-metrics dd').nth(1)).toContainText('1/60');
  await expect(page.locator('.review-metrics dd').nth(2)).toContainText('1/60');
