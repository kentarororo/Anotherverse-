...
| Choose one of three unlabeled hero beginnings; exclude discarded candidates; generate a balanced company as companions appear | `forge-roster.test.ts`; `director.test.ts`; `playwrite-slice.spec.ts`                           | Pass                     |
| Victory grants named monster materials; three-material weighted Forge is deterministic and persistent                         | `forge-roster.test.ts`; `foundation.spec.ts`; `full-campaign.spec.ts`                           | Pass                     |
...
| 100 seeded drafts: valid trios, role coverage, authored rules, and content-derived (not seed-derived) fingerprints            | `generation.test.ts`                                                                            | Pass                     |
| Typed grammar and unresolved-slot rejection                                                                                   | `grammar.test.ts`; `corpus-review.test.ts`                                                      | Pass                     |
| Archived 100-paragraph modern corpus remains parseable for regression comparison                                              | `corpus-review.test.ts`; `corpus-review.spec.ts`; `/?review=corpus`                             | Archive only             |
...
| One stable plain-English narration frame for every combat event type                                                          | `narrative.test.ts`; seven modules in `contentManifest`                                         | Pass                     |
| Four world-scoped operation encounters per realm plus choice-bound secret encounters align enemies and story ecology          | `mythic-content-boundary.test.ts`; `director.test.ts`; `progression.test.ts`                    | Pass                     |
...
| Authored results persist and bind into later chapters without changing their meaning                                          | `director.test.ts`                                                                              | Pass                     |
| Ten materials, forged counters, Bestiary confidence, and progression resources each create a meaningful decision              | `forge-roster.test.ts`; `progression.test.ts`; `engine.test.ts`; 20-turn browser test           | Partial; balance review  |
...
| Playback shows every action in order at 800 ms; units remain mobile-contained; canonical HP and cues match events             | `battle-playback.test.tsx`; `foundation.spec.ts`                                                | Pass                     |
| A sound opening plan materially outperforms an all-Aggressive, striker-Front, conserve-AP plan across 60 seeds                | `combat-balance.test.ts`                                                                        | Pass                     |
...
| -------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | ---------------------- |
| Read the production six-turn mythic slice aloud; every scene has clear cause, stakes, action, and result       | Follow `PLAYTEST.md` on the deployed build | Pending human playtest |
