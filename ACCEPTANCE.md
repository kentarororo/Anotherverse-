...
| Four core encounters plus three choice-bound secret encounters align choices, enemies, forecast, and Bestiary                   | `director.test.ts`; `progression.test.ts`; `full-campaign.spec.ts`                              | Pass                     |
...
| Ten materials, forged counters, Bestiary confidence, Rations, Coin, Dust, Renown, rank, relationships, and training affect play | `forge-roster.test.ts`; `progression.test.ts`; `engine.test.ts`; 20-turn browser test           | Pass                     |
