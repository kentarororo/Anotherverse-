...
| Fixed 100-paragraph review corpus, two live facts each, deterministic, no unresolved slots                                 | `corpus-review.test.ts`; `/?review=corpus`                                                      | Pass (structure)               |
...
| Autosave on creation/commit/equip/learn; corrupt/incompatible recovery; confirmation before replacement                    | `persistence.test.ts`; `foundation.spec.ts`                                                     | Pass                           |
| Title Settings, 1365×768 no-scroll loop, mobile sticky Commit, 125% text, reduced motion, keyboard focus return            | `foundation.spec.ts`                                                                            | Pass                           |
...
| GitHub Pages build uses `/Anotherverse-/` and deploys `dist` from `main`                                                   | `vite.config.ts`; `.github/workflows/pages.yml`                                                 | Configured; awaiting v0.4 push |

## Human gates

| Requirement                                                                                                    | How to verify                                              | Status                 |
| -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------- |
| Read aloud 100 fixed paragraphs: at least 95 natural and 85 coherent                                           | Deploy, open `/?review=corpus`, read and score all entries | Pending human review   |
